...
import os
from utils.models import (build_lstm_model, build_rf_model, 
                         build_ridge_model, build_xgboost_model,
                         build_transformer_model, build_rf_tuned,
                         build_xgboost_tuned,
                         compile_model, train_model, forecast_future)

def prepare_data(ticker, start_date='2015-01-01', end_date='2025-03-01', window_size=60):
...
def save_results(ticker, model_type, test_dates, y_test, y_pred, future_dates, future_pred):
    ...
    plt.plot(future_dates, future_pred, label='Forecasted Price', color='green', linestyle='dashed')
    plt.xlabel('Date')
    ...
    pd.DataFrame({
        "Date": future_dates,
        "Forecasted": future_pred.flatten()
    }).to_csv(f"data/{model_type}/{ticker.split('.')[0]}_forecast.csv", index=False)
...
def execute(ticker, model_type='lstm', window_size=60):
    ...
    if model_type == 'lstm':
        ...
        X_train_reshaped = X_train.reshape(X_train.shape[0], -1)
        X_test_reshaped = X_test.reshape(X_test.shape[0], -1)
    elif model_type == 'rf_tuned':
        model = build_rf_tuned()
        X_train_reshaped = X_train.reshape(X_train.shape[0], -1)
        X_test_reshaped = X_test.reshape(X_test.shape[0], -1)
    elif model_type == 'xgboost_tuned':
        model = build_xgboost_tuned()
        X_train_reshaped = X_train.reshape(X_train.shape[0], -1)
        X_test_reshaped = X_test.reshape(X_test.shape[0], -1)
    ...
    future_pred = forecast_future(model, last_window, scaler, model_type, window_size=window_size)
    future_dates = pd.date_range(start=df.index[-1] + pd.Timedelta(days=1), periods=30)
    
    save_results(ticker, model_type, test_dates, y_test, y_pred, future_dates, future_pred)
